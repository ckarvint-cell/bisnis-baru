...
export default function CheckoutPage() {
  ...
  function requestCheckout(e: React.FormEvent) {
    ...
    if (cart.length === 0) {
      setMessage("Keranjang masih kosong. Tambahkan produk terlebih dahulu.");
      return;
    }
  ...
  if (loading) {
    return <main className="flex min-h-screen items-center justify-center bg-[#f7f3f5] px-4 text-slate-900"><p className="text-sm font-medium">Memuat checkout...</p></main>;
  }
    ...
    <main className="min-h-screen bg-[#f7f3f5] text-slate-950">
      <header className="border-b border-rose-100 bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="text-2xl font-semibold">Data Pembayaran</h1>
            <p className="mt-1 text-sm text-slate-600">Isi data pengiriman dan upload bukti transfer.</p>
          </div>
          <Link href="/cart" className="rounded-md border border-rose-200 px-4 py-2 text-sm font-medium hover:bg-rose-50">Kembali ke Keranjang</Link>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-[1fr_380px]">
        <form onSubmit={requestCheckout} className="rounded-lg border border-rose-100 bg-white p-5 shadow-sm">
          ...
          <button disabled={saving || cart.length === 0} className="mt-5 rounded-md bg-slate-950 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-700 disabled:opacity-50">Konfirmasi Checkout</button>
        </form>
      ...
      {confirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">Semua produk di keranjang akan dibuat menjadi satu pesanan senilai {formatRupiah.format(total)}.</p>
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button onClick={confirmCheckout} disabled={saving} className="rounded-md bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:opacity-50">{saving ? "Menyimpan..." : "Ya, Checkout"}</button>
            </div>
          </div>
        </div>
      )}
